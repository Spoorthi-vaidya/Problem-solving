...
public class MovieStreaming extends MoviesList {
	...
	/*
	 * Functionality: Displays the available movie_names, movie IDs, movie
	 * length
	 */
	public void displayMovieList() {
		...
		System.out
				.println("Existing movieid, moviename and their corresponding length : \n");

		for (Map.Entry<Integer, MovieKeys> s : a) {
			System.out.println(s.getKey() + ":" + s.getValue().moviename + ":"
					+ s.getValue().movielength);
		}

	}

	/*
	 * Functionality: Checks whether the sum of two movielength of any movies
	 * equals to travel length
	 */
			...
			for (int m = i + 1; m < noofmovies; m++) {
				...
				if (movie_length[m][0] != movie_length[i][0]) {
					if ((l1 + l2) == travellength) {
						flag = true;
						System.out.println("[(" + movie_length[i][0] + ","
								+ movie_length[i][1] + ")" + " , " + "("
								+ movie_length[m][0] + "," + movie_length[m][1]
								+ ")]");
					}
	...
	/* Functionality: Checks whether the same length movies exits */
